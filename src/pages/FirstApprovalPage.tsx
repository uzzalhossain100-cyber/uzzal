...
import { FirstApprovalModal } from '@/components/modals/FirstApprovalModal';
import { Voucher } from '@/types/voucher';

const FirstApprovalPage = () => {
  const { vouchers, updateVoucherStatus, loggedInUser, voucherCreator } = useVouchers();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedVoucher, setSelectedVoucher] = useState<Voucher | null>(null);

  const vouchersForApproval = vouchers.filter((v) => v.status === 'submitted');
  ...
  const handleProcessApproval = (action: 'approve' | 'reject' | 'revert' | 'forward', reason?: string) => {
    if (!selectedVoucher) return;

    let newStatus;
    if (action === 'approve') {
      newStatus = selectedVoucher.type === 'ক্রেডিট ভাউচার' ? 'approved_check' : 'approved_1st';
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold mb-4 text-center text-gray-800">প্রথম অ্যাপ্রুভালের জন্য</h2>
        {vouchersForApproval.length === 0 ? (
          <p className="text-center text-gray-600">প্রথম অ্যাপ্রুভালের জন্য কোনো ভাউচার নেই।</p>
        ) : (
          <div className="overflow-x-auto">
            <Table className="min-w-full">
              ...
              <TableBody>
                {vouchersForApproval.map((voucher) => (
...
export default FirstApprovalPage;